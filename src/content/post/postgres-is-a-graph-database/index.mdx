...
When it comes to storing graph data structures we might reach for a database marketed for that usecase like Neo4J or Dgraph. Hold your horses!
...
A person can have friends, and those friends also have their own list of friends.
...
To store graph data structures in Postgres, we need to create two tables: one for nodes and one for edges. The nodes table will store information about each entity, while the edges table will store information about the relationships between entities.
...
Once we have stored our graph data structures in Postgres, we can query them using SQL statements. For example, to retrieve all nodes in the graph, we would execute the following SQL statement:
