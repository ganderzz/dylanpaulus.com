...
tags: ["Postgres", "Database", "Data Structures"]
date: "2023-03-16"
published: true
---
...
While Postgres is not generally thought of to store graph data structures, it has some built-in features that allow it to store and query graph data efficiently.
...
To give a real-world example, Facebook is a popular social media platform that uses a graph to represent people and their relationships.
A person can have friends, and those friends also have their list of friends.
Each person would be represented as a node, and each friendship would be represented as an edge.
Graphs can be used to model a lot of different applications like your
...
The `nodes` table we defined here has two columns: id and data. The id column is an auto-incrementing integer that serves as the primary key for the table. The data column is a string that stores any additional data associated with the node.
For this example, we're keeping it simple and only storing a string column, but in real-world applications, this table could have any number of columns.

The most important table in creating a graph data structure is the `edges` table:
...
Here, we are creating two columns, `previous_node` and `next_node`, that represent our relationships between nodes. Each of these columns represents a foreign key to a node.
In a graph an edge can only have one `previous_node` and `next_node` pairing, so we are using a composite primary key to ensure that each edge is unique.
...
<div class="flex flex-col items-center justify-center my-6">
  ![A graph representation of the SQL created
  above](postgres-is-a-graph-database/example-1.svg)
</div>
...
<figcaption class="text-sm my-4 text-center">
  Find all `nodes` that are connected to the node with `id` 1 (Bob's id).
</figcaption>

Looks like Bob is popular! But what if we want to know who Bob's friends are friends with?

Let's insert a few more nodes and edges to demonstrate this:
...
To query for all of Bob’s friends of friends we could extend the previous query to join the `edges` table again, but that would cause a maintenance nightmare having to manually join each “level” in the graph.

Fortunately, Postgres has a built-in feature that allows us to query graph data without having to know exactly how many joins we need: recursive queries. Recursive queries allow us to traverse the graph starting from a specific node and following its edges until some determined endpoint.

Writing a recursive query to find all of Bob’s friends and their friends we would write the following SQL:
...
In our example we want to start our query with Bob's friends, so we find the edges where Bob (id: 1) is the `previous_node` and select the `next_node`.
Then we will use the recursive case to recursively join the `edges` table to itself until we reach the end of Bob's graph.
Finally, we need to query the `nodes` that are associated with the edges from the recursive query to get each of Bob's friends' names.
...
By using features built into Postgres, we can easily store and query graph data structures.
If you are already using Postgres for your relational data, you can easily integrate graph data structures into your existing database without adding additional systems.
