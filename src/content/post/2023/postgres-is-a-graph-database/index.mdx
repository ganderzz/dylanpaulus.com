...
description: "You don't need Neo4j for graph data. Walking through nodes, edges, and recursive traversals in plain Postgres, with working SQL."
...
If you want to try the queries in this post against a populated table, the [`generate_series` walkthrough](/posts/easily-generate-mock-data-with-postgresql) covers how to spin up nodes and edges quickly. For more on the recursive CTE pattern used above, see [Recursive Query in SQL: What It Is, and How to Write One](/posts/recursive-query-in-sql-what-it-is-and-how-to-write-one).
