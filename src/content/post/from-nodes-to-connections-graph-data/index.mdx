---
title: "Unraveling Graph Structures: Exploring Adjacency Lists and Matrices"
series: "From Nodes to Connections"
tags: ["Data Structures", "Graphs"]
date: "2023-07-04"
published: false
---

In [Postgres: The Graph Database You Didn't Know You Had](/posts/postgres-is-a-graph-database) we covered how we can store graph data in a relational database.
We looked at one representation of a graph data structure called an adjacency list--a table with two columns representing how nodes connect.
The two columns are a reference to the <em>source</em> or <em>"from"</em> node to a <em>destiation</em> or <em>"to"</em> node.

<div class="lg:mt-8" />

<section class="full-bleed">
    <div class="lg:flex justify-between gap-4 max-w-7xl mx-auto px-4">
        <div class="w-full lg:w-1/2 text-center">
            <strong>Our Graph</strong>
            <img
            src="from-nodes-to-connections-graph-data/base_graph.svg"
            alt="A graph data structure with three nodes: a, b, and c. A has edges that connect to b and c."
            class="dark:invert mx-auto"
            />
        </div>

        <div class="w-full lg:w-1/2 mt-16 lg:mt-0 mx-auto">
            <strong>The Adjacency List</strong>
            <table class="w-full mx-auto">
                <thead>
                    <tr>
                        <th colspan="2">Edges</th>
                    </tr>
                    <tr>
                        <th>From</th>
                        <th>To</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>A</td>
                        <td>B</td>
                    </tr>
                    <tr>
                        <td>A</td>
                        <td>C</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

</section>

An adjacency list is a simple and efficient way to store graph data, but there are many more ways to represent graphs.
In this article we'll explore one of these alternatives called the adjacency matrix.

## Adjacency Matrix

An adjacency matrix is a table with a row and column for each node in the graph.
To represent the edges we store a 1 in a column for a connection and a 0 for no connection.
Using the same graph as above, an adjacency matrix would look like this:

<div class="lg:mt-8" />
<section class="full-bleed">
    <div class="lg:flex justify-between gap-4 max-w-7xl mx-auto px-4">
        <div class="w-full lg:w-1/2 text-center">
            <strong>Our Graph</strong>
            <img
            src="from-nodes-to-connections-graph-data/base_graph.svg"
            alt="A graph data structure with three nodes: a, b, and c. A has edges that connect to b and c."
            class="dark:invert mx-auto"
            />
        </div>

        <div class="w-full lg:w-1/2 mt-16 lg:mt-0 mx-auto">
            <strong>The Adjacency Matrix</strong>
            <table class="w-full mx-auto">
                <thead>
                    <tr>
                        <th></th>
                        <th>A</th>
                        <th>B</th>
                        <th>C</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>

        <td class="bg-secondary-300">A</td>
                        <td>0</td>
                        <td>1</td>
                        <td>1</td>
                    </tr>
                    <tr>
        <td class="bg-secondary-300">B</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                    </tr>
                        <tr>
        <td class="bg-secondary-300">C</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>

</section>
