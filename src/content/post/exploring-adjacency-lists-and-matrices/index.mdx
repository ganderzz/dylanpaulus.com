...
For unidirected graphs, graphs where all connections are bidirectional, the setup is the same to create the adjacency matrix.
The only difference is our graph has no explicit _direction_. If `A` connections to `B`, then `B` also connects to `A`. 
...
<section class="full-bleed">
    <div class="lg:flex justify-between gap-4 max-w-7xl mx-auto px-4">
        <div class="w-full lg:w-1/2 text-center">
            <strong>Our Graph</strong>
            <img
            src="from-nodes-to-connections-graph-data/unidirect.svg"
            alt="A graph data structure with three nodes: a, b, and c. A has edges that connect to b and c."
            class="dark:invert mx-auto"
            />
        </div>
...
A cool property emerges from unidirected graphs. Can you spot a pattern in the table above?
...
<section class="full-bleed">
    <div class="lg:flex justify-between gap-4 max-w-7xl mx-auto px-4">
<img
  src="from-nodes-to-connections-graph-data/table.svg"
  alt="A graph data structure with three nodes: a, b, and c. A has edges that connect to b and c."
  class="dark:invert mx-auto"
/>
</div>
...
Symmetric matrices appear in all unidirected graphs, but not DAGs!
...
as a hash table. In our example graph above, if we wanted to check if *A* has an edge to *B* it would be as simple as checking the `[B][A]` position
in the matrix.
