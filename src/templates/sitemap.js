...
export default function SiteMap({ data }) {
  ...
  return (
    <Layout>
      <SEO description="Sitemap" title="Sitemap" keywords={Object.keys((tagsList || {}))} />
      ...
      {Object.keys(tagsList).map(key => (
        <Link
          key={key}
          to={`/tags/${key}`}
          className="focus:border-2 focus:border-grey-dark hover:opacity-100 m-2 hover:bg-gray-700 hover:text-white text-base rounded bg-gray-700 p-2 text-white no-underline font-semibold mr-2 opacity-75"
        >
          {key} ({tagsList[key] || 0})
        </Link>
      ))}
