...
import { LOGO } from "./logo-img.tsx";
import { generateCard } from "./card.tsx";
import { generateText } from "./text.tsx";

const DIRNAME = path.dirname(fileURLToPath(import.meta.url));
const CONTENT_PATH = path.resolve(DIRNAME, "../src/content/post");

const readFileAsync = async (file: string): Promise<ArrayBuffer> => {
...
async function getContent() {
    ...
    for (const directory of directories) {
        ...
        const nestedDirectories = await promises.readdir(path.join(CONTENT_PATH, directory));

        for (const yearDirectory of nestedDirectories) {
            const contentDir = path.join(CONTENT_PATH, directory, yearDirectory);
            const files = await promises.readdir(contentDir);
            ...
            for (const file of files) {
                ...
                const imagePath = imageMatch
                    ? path.join(contentDir, imageMatch[1])
                    : null;
    
                if ((match?.length ?? 0) > 0) {
                    results.push({
                        fileDirectory: yearDirectory,
                        title: match?.[1] ?? "",
                        image: imagePath,
                    });
                }
...
async function generateThumbnail({
    ...
    interBold,
    interRegular,
}: { fileDirectory: string; title: string; image: any; interBold: ArrayBuffer, interRegular: ArrayBuffer}) {
    console.log(`Creating Thumbnail for ${title}...`);

    const svg = await generateCard();
    const text = await generateText({ title, interBold, interRegular });
